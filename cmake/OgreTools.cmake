...
#
# Description:
# CMake package config file for libPR. Some useful tools for Ogre3D. Freely inspired from # the one of the ViSP library. 
#
# Authors:
# Guillaume Caron
#
...
macro(pr_ogre_find_plugin_lib_per PLUGIN)
...
macro(pr_create_ogre_plugin_config_file)
  set(PER_HAVE_OGRE_PLUGINS_PATH "${PER_BINARY_DIR}/data/ogre-simulator" CACHE INTERNAL "Ogre plugins location")
  ...
  if(NOT OGRE_PLUGIN_DIR_REL AND NOT OGRE_PLUGIN_DIR_DBG)
    pr_ogre_find_plugin_lib_per(RenderSystem_Direct3D9)
    pr_ogre_find_plugin_lib_per(RenderSystem_Direct3D10)
    pr_ogre_find_plugin_lib_per(RenderSystem_Direct3D11)
    pr_ogre_find_plugin_lib_per(RenderSystem_GL)
    pr_ogre_find_plugin_lib_per(RenderSystem_GLES)
    pr_ogre_find_plugin_lib_per(Plugin_ParticleFX)
    pr_ogre_find_plugin_lib_per(Plugin_BSPSceneManager)
    pr_ogre_find_plugin_lib_per(Plugin_CgProgramManager)
    pr_ogre_find_plugin_lib_per(Plugin_PCZSceneManager)
    pr_ogre_find_plugin_lib_per(Plugin_OctreeSceneManager)
    pr_ogre_find_plugin_lib_per(Plugin_OctreeZone)
  ...
  if(OGRE_PLUGIN_DIR_REL)
    ...
    file(WRITE "${PER_HAVE_OGRE_PLUGINS_PATH}/plugins.cfg" ${PLUGINS_CONTENT_REL})
  endif()
  ...
  if(OGRE_PLUGIN_DIR_DBG)
    ...
    file(WRITE "${PER_HAVE_OGRE_PLUGINS_PATH}/plugins_d.cfg" ${PLUGINS_CONTENT_DBG})
  endif()
endmacro()

function(pr_set_ogre_media)
  ...
  # Because in that case, OGRE_MEDIA_DIR should be set first to PER_HAVE_OGRE_RESOURCES_PATH
  #  (for the "make all" case) then to PER_INSTALL_DIR_OGRE_RESOURCES (for the "make install" case)
  if(NOT OGRE_MEDIA_DIR)
  ...
  # path to avoid recopy of the plugins in PER.
  ...
  if(ogre_plugings_cfg_exists)
    set(PER_HAVE_OGRE_PLUGINS_PATH "${ogre_plugings_cfg_exists}" CACHE INTERNAL "Ogre plugins location")
  else(NOT ogre_plugings_cfg_exists)
    # If no plugins.cfg file is found, we create one with absolute path

    # case 1: normal case
    #--------------
    pr_create_ogre_plugin_config_file()
    ...
    if(UNIX)
      if(OGRE_PLUGIN_DIR_REL)
        install(FILES
          ${PER_HAVE_OGRE_PLUGINS_PATH}/plugins.cfg
          DESTINATION ${CMAKE_INSTALL_LIBDIR}/per/data/ogre-simulator
      ...
      if(OGRE_PLUGIN_DIR_DBG)
        install(FILES
          ${PER_HAVE_OGRE_PLUGINS_PATH}/plugins_d.cfg
          DESTINATION ${CMAKE_INSTALL_LIBDIR}/per/data/ogre-simulator
          PERMISSIONS OWNER_READ GROUP_READ WORLD_READ OWNER_WRITE
          COMPONENT dev
        )
  ...
  if(OGRE_MEDIA_NOT_AVAILABLE)
    file(COPY modules/ar/data/ogre-simulator/media DESTINATION ${PER_BINARY_DIR}/data/ogre-simulator)
  endif()

  if(ogre_resources_cfg_exists)
    set(PER_HAVE_OGRE_RESOURCES_PATH "${ogre_resources_cfg_exists}" CACHE INTERNAL "Ogre resources location")
  else()
    # Here we create a resources.cfg if it was not found

    # we create a resources.cfg file for vpAROgre.cpp
    # case 1: normal case
    #         If OGRE_MEDIA_DIR is not found, we set it to PER_HAVE_OGRE_RESOURCES_PATH in order to use
    #         the minimal requested media to run the examples
    #--------------
    set(PER_HAVE_OGRE_RESOURCES_PATH "${PER_BINARY_DIR}/data/ogre-simulator" CACHE INTERNAL "Ogre resources location")

    if(OGRE_MEDIA_NOT_AVAILABLE)
      set(OGRE_MEDIA_DIR ${PER_HAVE_OGRE_RESOURCES_PATH}/media)
    endif()

    # Here we add all the subdirs in @OGRE_MEDIA_DIR@/* as resource location.
    pr_get_relative_subdirs(media_subdirs ${OGRE_MEDIA_DIR})
    ...
    configure_file(
      ${PER_SOURCE_DIR}/cmake/templates/resources.cfg.in
      ${PER_HAVE_OGRE_RESOURCES_PATH}/resources.cfg
      IMMEDIATE @ONLY
    )

    # case 2: install or packaging case
    #         If OGRE_MEDIA_DIR is not found, we set it to PER_INSTALL_DIR_OGRE_RESOURCES in order to use
    #         the minimal requested media to run the examples
    #--------------
    if(UNIX)
      set(PER_INSTALL_DIR_OGRE_RESOURCES "${CMAKE_INSTALL_PREFIX}/${CMAKE_INSTALL_DATAROOTDIR}/per-${PER_VERSION}/data/ogre-simulator")
    else()
      set(PER_INSTALL_DIR_OGRE_RESOURCES "${CMAKE_INSTALL_PREFIX}/data/ogre-simulator")
    endif()
    # make the var global
    set(PER_INSTALL_DIR_OGRE_RESOURCES ${PER_INSTALL_DIR_OGRE_RESOURCES} CACHE INTERNAL "Ogre media install dir")

    if(OGRE_MEDIA_NOT_AVAILABLE)
      set(OGRE_MEDIA_DIR ${PER_INSTALL_DIR_OGRE_RESOURCES}/media)
    endif()
    ...
    if(UNIX)
      configure_file(
        ${PER_SOURCE_DIR}/cmake/templates/resources.cfg.in
        ${PER_BINARY_DIR}/unix-install/resources.cfg
        IMMEDIATE @ONLY
      )
      install(FILES
        ${PER_BINARY_DIR}/unix-install/resources.cfg
        DESTINATION ${CMAKE_INSTALL_DATAROOTDIR}/per-${PER_VERSION}/data/ogre-simulator
        PERMISSIONS OWNER_READ GROUP_READ WORLD_READ OWNER_WRITE
        COMPONENT dev
      )
      if(OGRE_MEDIA_NOT_AVAILABLE)
        install(DIRECTORY
          ${PER_BINARY_DIR}/data/ogre-simulator/media
          DESTINATION ${CMAKE_INSTALL_DATAROOTDIR}/per-${PER_VERSION}/data/ogre-simulator
      ...
      configure_file(
        ${PER_SOURCE_DIR}/cmake/templates/resources.cfg.in
        ${PER_BINARY_DIR}/win-install/resources.cfg
        IMMEDIATE @ONLY
      )
      install(FILES
        ${PER_BINARY_DIR}/win-install/resources.cfg
