...
/*!
 \class PER_EXPORT prRegularlySampledCSImage prRegularlySampledCSImage.h
 <per/prRegularlySampledCSImage.h> \brief Class defining the image place as a
 set of spherical points in Cartesian coordinates lying
 *      on a unit sphere and regularly sampled, in the geodesic meaning
 */
template <typename T>
class PER_EXPORT prRegularlySampledCSImage : public prAcquisitionModel<T> {
public:
  /*!
   * \fn prRegularlySampledCSImage(unsigned int _subdivLevels = 3)
   * \brief Constructor of a prRegularlySampledCSImage object (CS stands for
   * Cartesian Spherical) : initializes the number of subdivision level of the
   * sphere, i.e. the resolution of the spherical image, and loads the spherical
   * points.
   */
  prRegularlySampledCSImage(unsigned int _subdivLevels = 3)
      : subdivLevels(_subdivLevels) //, XS(NULL) //, nbSamples(0), bitmap(NULL),
                                    // bitmapf(NULL), isBitmapfSet(false)
  {
    loadSphere();
  }
   ...
   * \brief Destructor of the prRegularlySampledCSImage object : frees memory of
   * grayscale intensities (raw and interpolated) and spherical points
   * coordinates array.
   */
  ...
  /*!
   * \fn int loadSphere()
   * \brief Allocates and fill the array of points coordinates as well as the
   * array of "pixels" (not set in that function since needs data) \return  0 if
   * the spherical points coordinates are well loaded -1 if the number of
   * subdivision level is too high
   */
  // NOTE: this function is not used in the current implementation, this is the
  // previous one, loading the .h int loadSphere()
  // {
  ...
  //     const prCartesian3DPointVec *pt_XS_src =
  //     prData::samplesCoordinates[subdivLevels]; float inorme = 1.0f;
  //     for(unsigned long i = 0 ; i < nbSamples ; i++, pt_XS++, pt_XS_src++)
  //     {
  //         //renormalize coordinates (might be necessary due to the text
  //         exportation inorme
  //         = 1.0f/sqrt(pt_XS_src->get_X()*pt_XS_src->get_X() +
   ...
   *
   * \brief Allocates and fill the array of points coordinates as well as the
   * array of "pixels" (not set in that function since needs data)
   *
   * \return  0 if
   * the spherical points coordinates are well loaded -1 if the number of
   * subdivision level is too high
  ...
  int loadSphere() {
    ...
    icoSampler.loadDelaunaySamples(this->subdivLevels);
  ...
  /*!
   * \fn int buildFromTwinOmni(vpImage<T> & I, prStereoModel & stereoCam,
   * vpImage<unsigned char> *Mask = NULL) \brief Builds the pixels map of the
   * spherical image from the acquired dual omni image data
   ...
   * \param Mask the pointer to a Mask image in order to ignore some pixels (if
   * NULL, every pixel of the input image is considered) \return  0 if the
   * spherical image is well built -1 if there is no spherical image points
   * loaded
   */
  int buildFromTwinOmni(vpImage<T> &I, prStereoModel &stereoCam,
    ...
    std::memcpy(MaskS.bitmap, Mask->bitmap,
                Mask->getHeight() * Mask->getWidth() * sizeof(unsigned char));
    ...
    prCartesian3DPointVec *pt_XS = (prCartesian3DPointVec *)ge;
    unsigned int icam = 0, imWidth = I.getWidth(), imHeight = I.getHeight(), i,
                 j;
    prPointFeature P;
        ...
        if ((u >= 0) && (v >= 0) && (u < (imWidth - 1)) &&
          ...
          case IMAGEPLANE_BILINEAR:
            ...
            if ((Mask != NULL)) {
              ...
              *pt_bitmapf = I[i][j] * unmdv * unmdu + I[i + 1][j] * dv * unmdu +
                            I[i][j + 1] * unmdv * du +
                            I[i + 1][j + 1] * dv * du;
              // consider = true;
            }
  ...
  /*!
   * \fn int toTwinOmni(vpImage<T> & I_r, vpPoseVector & r, prStereoModel &
   * stereoCam, vpImage<unsigned char> *Mask = NULL) \brief Projects the
   * spherical image to a dual omni image (mainly for visualization purpose)
   * TODO: to generalize for other sensor types than the dual omni
   * \param I_r (output) the output image of which pixels are of type T
   * \param r the pose vector of the frame change to apply to the spherical
   * image before projection in the dual omni image plane \param stereoCam the
   * dual omni camera intrinsic and extrinsic parameters \param Mask the pointer
   * to a Mask image in order to ignore some pixels (if NULL, every pixel of the
   * input image is considered ; if not NULL, masked pixels are not changed)
  ...
  int toTwinOmni(vpImage<T> &I_r, vpPoseVector &r, prStereoModel &stereoCam,
    ...
    prCartesian3DPointVec XSs;
    prCartesian3DPointVec *pt_XS = (prCartesian3DPointVec *)ge;
    unsigned int icam = 0, imWidth = I_r.getWidth(), imHeight = I_r.getHeight(),
                 i, j;
    prPointFeature P;
    double u, v, du, dv, unmdu, unmdv;
   ...
   * vpImage<unsigned char> *Mask = NULL) \brief Builds the pixels map of the
   * spherical image from the acquired equirectangular image data
   * TODO: to generalize for other sensor types than the equirectangular by
   * merging with buildFromTwinOmni \param I the input image of which pixels are
   * of type T \param equiCam the dual omni camera intrinsic and extrinsic
   * parameters \param Mask the pointer to a Mask image in order to ignore some
   * pixels (if NULL, every pixel of the input image is considered) \return  0
   * if the spherical image is well built -1 if there is no spherical image
   * points loaded
   */
      ...
      if ((u >= 0) && (v >= 0) && (u < (imWidth - 1)) && (v < (imHeight - 1))) {
        ...
        case IMAGEPLANE_BILINEAR:
          ...
          if ((Mask != NULL)) {
            ...
            *pt_bitmapf = I[i][j] * unmdv * unmdu + I[i + 1][j] * dv * unmdu +
                          I[i][j + 1] * unmdv * du + I[i + 1][j + 1] * dv * du;
            // consider = true;
          }
  ...
  /*!
   * \fn int toEquiRect(vpImage<T> & I_r, vpPoseVector & r, prEquirectangular &
   * equiCam, vpImage<unsigned char> *Mask = NULL) \brief Projects the spherical
   * image to an equirectangular image (mainly for visualization purpose)
   * TODO: to generalize for other sensor types than the equirectangular by
   * merging with toTwinOmni \param I_r (output) the output image of which
   * pixels are of type T \param r the pose vector of the frame change to apply
   * to the spherical image before projection in the dual omni image plane
   * \param equiCam the equirectangular camera intrinsic and extrinsic
   * parameters \param Mask the pointer to a Mask image in order to ignore some
   * pixels (if NULL, every pixel of the input image is considered ; if not
   * NULL, masked pixels are not changed) \return  0 if the spherical image is
   * well built -1 if there is no spherical image points loaded
   */
  int toEquiRect(vpImage<T> &I_r, vpPoseVector &r, prEquirectangular &equiCam,
                 vpImage<unsigned char> *Mask = NULL) {
    if (nbSamples == 0)
      return -1;
   ...
   * \brief Extract the sample position and value (that might have been
   * interpolated) at given index \param i the index at which to extract the raw
   * \param pXS (output) the output coordinates of the sample
  ...
  /*!
   * \fn prRegularlySampledCSImage<T> &operator=(const
   * prRegularlySampledCSImage<T> &RSCSI) \brief = operator override to copy
   * prRegularlySampledCSImage object
   ...
   */
  prRegularlySampledCSImage<T> &
  operator=(const prRegularlySampledCSImage<T> &RSCSI) {
    ...
    bitmap = new T[nbSamples];
    std::memcpy(bitmap, RSCSI.bitmap,
                nbSamples * sizeof(T)); // OK si T est un type de base
    ge = new prCartesian3DPointVec[nbSamples];
    ...
    std::memcpy(MaskS.bitmap, RSCSI.MaskS.bitmap,
                RSCSI.MaskS.getHeight() * RSCSI.MaskS.getWidth() *
   ...
   */
  unsigned int getSampleDim() { return 3; }

  int changeSampleFrame(unsigned long &ns, vpHomogeneousMatrix &dMc,
                        prCartesian3DPointVec &XS, double &rho) {
  ...
  vpImage<T> getSampledImage() { return I_req; }
  ...
  // prCartesian3DPointVec *XS; /*!< array of Cartesian spherical points
  // coordinates */
